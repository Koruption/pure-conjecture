...
$$\mathbb{E}\left(\mu(T^n) \right)=\lim_{n\to\infty} \frac{1}{nk}\sum_{i=0}^n \sum_{p\in\omega}^{k} |\text{det J}_p|$$
...
$$\mu(T^{i+1})-\mu(T^i)=\left(\frac{\mu(T^{i+1})-\mu(T^i)}{T^{i+1}-T^i}\right)\left(T^{i+1}-T^i\right)$$
...
$$\frac{d}{dx^i}\mathbb{E}\left(\mu(T(x_i))\right)=\mathbb{E}\left[\frac{d\mu}{d T} \frac{dT}{dx^i} \right]$$
...
$$T(\theta, \phi)=(\theta+\epsilon sin\phi,\phi+\delta sin\theta)$$
...
$$\theta_0=0, \theta_1=\frac{\pi}{2},\,\,\,\phi_0=\pi,\phi_1=2\pi$$
...
$$\nabla\,\mathbb{E}\left(\mu(T(x_i))\right)=0$$
